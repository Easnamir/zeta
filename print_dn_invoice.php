...
if(date('m')<4){
		...
		if(isset($_GET['fun']) && $_GET['fun']=='printInvoicedn'){
			 $dn_id = $_GET['id'];
...
from POPS_DN_DETAILS a 
left join POPS_DEP_DETAILS b  on b.DEPARTMENT_NAME=a.VEND_NAME where DN_DETAILS_PK='$dn_id'
...
from POPS_DN_DETAILS a join POPS_VEND_DETAILS b on a.VEND_CODE=b.VEND_CODE
join POPS_DEP_DETAILS c  on c.DEPARTMENT=b.DEPARTMENT where DN_DETAILS_PK='$dn_id'";
// exit;
				$data=[];
				...
				while($row2 = sqlsrv_fetch_array($stmt2,SQLSRV_FETCH_ASSOC)){
					...
					$pan = $row2['PAN_NO'];
					$cn_date = $row2['DN_DATE']->format('d M Y');
					$cn_month = $row2['MONTH']->format('F,Y');
					
					$inv_no =strtoupper($row2['DN_NO']);
        ...
        <h3 class='center_text under_line'>Debit Note</h3>
