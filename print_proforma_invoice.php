...
if(date('m')<4){
		...
		extract($_GET);
			  $sql2="select a.*,b.*,c.*,b.gst_no from POPS_PROFORMA_INVOICE a
				 join POPS_VEND_DETAILS b on b.VEND_CODE=a.VEND_CODE 
				 join POPS_PRICE_MASTER c on a.BRAND_CODE=c.BRAND_CODE where INVOICE_NUMBER='$pid'";
				...
				while($row2 = sqlsrv_fetch_array($stmt2,SQLSRV_FETCH_ASSOC)){
					...
					$po_num = $inv.str_pad($pid,6,'0',STR_PAD_LEFT);
					
					$liq_type=$row2['LIQUOR_TYPE_CD'];
					$GST_NO = $row2['GST_NO'];
					$TIN_NO = $row2['TIN'];
          $FSSAI = $row2['FSSAI'];
					$IS_DUTY = $row2['IS_DUTY_FREE'];
				}
      //   	print_r($row2);
						// exit;
        ...
        $contact
          ...
          <br><b>GST No : </b> $GST_NO <br><b>FSSAI No :</b> $FSSAI
				...
				for($i=0;$i<$row_count;$i++){
					$qty = $data[$i]['QUANTITY']*$data[$i]['PACK_SIZE'];
					...
					$mrp = $data[$i]['MRP'];
					 ...
					 $cost = $wsp+$custom;
					$rate = number_format($cost*$packSize,2,'.','');
					
					$amount = number_format($rate*$data[$i]['QUANTITY'],2,'.','');
					
					 $html .=("<tr><td class='no-top'>".($i+1)."</td><td colspan=4 class='no-top-left'>".$data[$i]['BRAND_NAME']."</td><td align=center  >".$data[$i]['SIZE_VALUE']."</td>
					 <td class='no-top-left' >".$data[$i]['MRP']."</td><td class='no-top-left' >".$data[$i]['QUANTITY']." $bottles</td>
					 <td class='no-top-left' >".$rate."</td><td class='no-top-left' >".$data[$i]['EXCISE_PRICE']*$packSize."</td><td align=right class='no-top-left'>".$amount."</td><tr>");
				...
				$html .="<table border=1 width=100% cellspacing=0 cellpadding=6 style='font-size: 12px'>
					...
					<br><b>&nbsp; Order No  :</b> $po_num &nbsp;&nbsp;&nbsp;&nbsp; <br>&nbsp; <b>$duty_msg</b>
				...
				<tr><td colspan=2>
				<table border=1 cellspacing=0 cellpadding=1 width=100%>
				<tr><th width=6%>Sno.</th><th colspan=4>Particulars</th><th >Size(ml)</th><th >MRP</th><th width=10%>Quantity</th><th width=10%>WSP P/C</th><th width=10%>Excise </th><th width=10%>Custom Duty</th><th width=10%>Amount</th></tr>";
				...
				for($i=0;$i<$row_count;$i++){
					...
					$amount = number_format($rate*$data[$i]['QUANTITY'],2,'.','');
					 ...
					 $html .=("<tr><td class='no-top'>".($i+1)."</td><td colspan=4 class='no-top-left'>".$data[$i]['BRAND_NAME']."</td><td align=center  >".$data[$i]['SIZE_VALUE']."</td>
					 <td class='no-top-left' >".$data[$i]['MRP']."</td><td class='no-top-left' >".$data[$i]['QUANTITY']." $bottles</td>
					 <td class='no-top-left' >".$wsp*$packSize."</td><td class='no-top-left' >".$data[$i]['EXCISE_PRICE']*$packSize."</td><td class='no-top-left' >".$custom*$packSize."</td><td align=right class='no-top-left'>".$amount."</td><tr>");
					 
					 $sum_quantity += $data[$i]['QUANTITY'];
					 $wsp_cal += round($data[$i]['WSP'],2)*$qty;
					 $revnue_exc += round($data[$i]['WSP']*$rate_rc,2)*$qty;
					 $sum_total_amount += $amount;
					 $sum_custom += $custom*$qty;
				}
				$revnue_exc=0;
				// updated here
				// updated here again to zeta
				...
				if($IS_DUTY){
				$html .="<tr><td class='only-left' align=right colspan=6>Less</td><td class='no-border' align=right colspan=5>C.D. Under SFIS (Debited From License) :</td><td align=right class='no-top'><p>".number_format($sum_custom, 2, '.', '')."</p></td></tr>";
				}
				else{
					$sum_custom=0;
				}
				$html .="<tr><td class='no-top no-right' align=left colspan=10><p><b>Amount (in words) :</b><em> ".ucfirst(digit_to_word($net_payable-$sum_custom))." Only</em></p></td><td class='no-top no-right no-left' align=right colspan=1><b>Total:</b><td align=right class='no-top'><b>".number_format((float)($net_payable-$sum_custom), 2, '.', '')."</b></td></tr>";
...
while($i<9){
					 $html .=("<tr><td class='no-top-bottom no-right' colspan=10>&nbsp;</td><tr>");
					 $i++;
				}	
