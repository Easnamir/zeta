...
const showCurrentSaleList = () => {
	...
	var totalQuantity=0;
	current_sale_array.map(row=>{
		html+='<tr id="'+ row.SKU_FK +'"><td>'+ i++ +'</td><td class="min-text">...</td><td class="mid-text">'+ row.BRAND_NAME +'</td><td>'+ row.SIZE_VALUE +'</td><td>'+ row.RETAIL_SELLING_PRICE +'</td><td>'+ row.MRP +'</td><td contenteditable="true" onblur="updateCurrentSaleList(this.parentNode.id,this.innerText)">'+ row.QUANTITY +'</td><td>'+ row.AMOUNT +'</td><td><a href="" onclick="deleteItemSaleList(event,this.id)" id="'+ row.UNIQUE_ID+'">X</a></td></tr>';
...
const appendToSalesList = (event,id) => {
	event.preventDefault();
	if(!brand_add){
		alert("You can't add brand in old Invoice. Please refresh page and add brand in new Invoice!!");
		return false;
	}
	...
	var selling = 0;
	const x = new Date(sale_date);
	const y = new Date('2022-02-22');
	// console.log(data);
	var SALE = 0;
	if(x>=y){
		selling =(data[0].RETAIL_SELLING_PRICE)?parseFloat(data[0].RETAIL_SELLING_PRICE):parseFloat(data[0].TOTAL_MRP_AMOUNT);
	}
	else{
		selling = parseFloat(data[0].TOTAL_MRP_AMOUNT);
	}
	object = {
		SKU_FK: data[0].SKU_FK,
		GTIN: data[0].GTIN,
		QUANTITY: SALE,
		HCR_QTY: data[0].HCR_QTY,
...
const showCurrentSalesList = () => {
	...
	invertory_sale_list.map(row=>{
		html+='<tr id="'+ row.SKU_FK+'"><td>'+ i++ +'</td><td class="mid-text">'+ row.BRAND_NAME +'</td><td>'+ row.SIZE_VALUE +'</td><td>'+ row.COUNTER_OPENING +'</td><td>'+ row.QUANTITY +'</td><td contenteditable="true" onblur="updateSaleInventoryList(this.parentNode.id)">'+ row.COUNTER_CLOSING +'</td><td>'+ (row.RETAIL_SELLING_PRICE) +'</td><td>'+ (row.TOTAL_AMOUNT) +'</td><td>'+(parseFloat(row.MRP)*parseInt(row.QUANTITY))+'</td><td class="mid-text"><a href="" onclick="deleteSaleItemList(event,this.parentNode.parentNode.id)">X</a></td></tr>';
		grandtotal = grandtotal+parseFloat(row.TOTAL_AMOUNT);
		totalQuantity=totalQuantity+parseInt(row.QUANTITY);
		mrp_amount=mrp_amount+parseInt(row.QUANTITY)*parseFloat(row.MRP);

	});

	html+='<tr><td colspan="4" style="text-align:right; padding-right:10px;"> <b>Total Quantity</td><td>'+totalQuantity+'</td><td colspan=2>Total Amount</td><td>'+grandtotal.toFixed(2)+'</td><td colspan=2>'+mrp_amount.toFixed(2)+'</td></tr>'
...
const updateSaleInventoryList =(id)=>{
	...
	if(counter_opening < counter_closing){
		...
		var hcr_qty=0;
		for(var i=0;i<invertory_sale_list.length;i++){
			
			if(invertory_sale_list[i].HCR_QTY>sale && invertory_sale_list[i].SKU_FK==id){
				count_status=true;
				hcr_qty = invertory_sale_list[i].HCR_QTY;
			}
		}
	...
	if(count_status){		
		cells[5].innerHTML='';
		cells[5].focus();
		document.getElementById('warning').innerText='Sale Quantity must be more than: '+hcr_qty;
		document.getElementById('warning').style.display='inline-block';
		sale=0;
		// console.log(document.getElementById('warning'))
		// console.log(invertory_sale_list[i])
			return false;
	}
	else{
...
const submitSalesItems = () =>{
	...
	if(inv_lenth < 1){
		...
		document.getElementById('sale_data').value=JSON.stringify(invertory_sale_list);
  ...
  xhttp.open("POST", "issue_sale_query.php?date="+date+"&invoice="+invoice, true);
   xhttp.send(data);
 ...
 function getInvoiceDataForSale (invoice) {
	...
	var sale_date = document.getElementById('sale_date');
	    ...
	    if (this.readyState == 4 && this.status == 200) {
	     ...
	     showCurrentSalesList();
	     document.getElementById('invoice_id1').value=invoice;
