...
foreach($data as $value){
	...
	foreach($brands as $key=> $value){
		...
		foreach($value as $size){
		  ...
		  $sqltp = "SELECT
	BRAND_NAME,
	MAX(CASE WHEN (SIZE_VALUE = $size) THEN CASE_QUANTITY ELSE 0 END) AS case_".$size.",
		MAX(CASE WHEN (SIZE_VALUE = $size) THEN (CUSTOM_DUTY+WSP)*BOTTLE_QUANTITY ELSE 0 END) AS wsp_".$size.",
	
  
  
		MAX(CASE WHEN (SIZE_VALUE = $size) THEN MRP*BOTTLE_QUANTITY ELSE 0 END) AS MRP_".$size.",
	
  
			MAX(CASE WHEN (SIZE_VALUE = $size) THEN EXCISE_DUTY*BOTTLE_QUANTITY ELSE 0 END) AS EXCISE_".$size."
	
	FROM
	POPS_DISPATCH_ITEMS WHERE TP_NO='$tp' and BRAND_NAME='$key' and SIZE_VALUE='$size' group by BRAND_NAME";
	// echo "<br>";
...
->setCellValueByColumnAndRow(1,5,"Sno")
	...
	$sql1="select PO_NO,INVOICE_NO,BRAND_NAME,SIZE_VALUE,sum(CASE_QUANTITY) as CASE_QUANTITY
	...
	group by PO_NO,INVOICE_NO,BRAND_NAME,SIZE_VALUE,WSP,CUSTOM_DUTY,EXCISE_DUTY,PACK_SIZE";
